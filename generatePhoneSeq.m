function sentence = generatePhoneSeq(num_phones)

stats = loadStats(1);

meanrate = stats.phone_dur.stats(1);
stdrate = stats.phone_dur.stats(2);
rate = normrnd(meanrate, stdrate);

phone_mc = dtmc(stats.phone_trans.t_prob);
phone_seq = sample(phone_mc, num_phones);
for p = 1:length(phone_seq)
    duration = class
end

time = 0;
p0 = randsample(total_phones, 1, true, phone_trans(1, :));
